...
- [x] Breadcrumb nav matches BreadcrumbList JSON-LD on all inner pages — all 13 inner-page templates covered: local-service (already correct), city, state, pricing, location-pathways, location-blog listing, location-blog post, service detail, global pathway, franchise, blog post, science article, research article; every template has matching visible nav + JSON-LD with "Home" as position 1
