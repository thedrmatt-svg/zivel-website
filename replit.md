...
- 2026-02-19: Abstract gradient hero — replaced stock photo hero with layered radial gradients (warm amber bottom-left, cool steel blue upper-right, gold center), two animated SVG wave shapes at bottom (amber + blue) with `zv-wave-drift` keyframe animation, dark overlay for text contrast; `.zv-hero-gradient` class with critical CSS overrides (transparent bg, absolute positioning fix, pseudo-element suppression); prefers-reduced-motion disables wave animation
- 2026-02-19: Blog auto-generated Table of Contents — `extractHeadings()` + `slugify()` helpers, "In This Article" ordered list with gold accents and h3 indentation, anchor IDs with `scroll-mt-28` on all heading blocks, `articleSection` array in Article schema for SEO
