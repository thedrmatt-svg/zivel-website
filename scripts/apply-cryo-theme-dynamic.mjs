...
`;
  ...
  const slugMatch = pageSrc.match(/const\s*\{\s*slug\s*\}\s*=\s*await\s+params\s*;/m);
  if (slugMatch) {
    return pageSrc.replace(slugMatch[0], slugMatch[0] + insert);
  }

  // Fallback: find function body start
  const fnMatch = pageSrc.match(/export\s+default\s+async\s+function\s+\w+[^{]*\{/m);
  if (fnMatch) {
...
function addWrapperToRootTag(pageSrc){
  ...
  // Remove old hardcoded cryo attributes first
  nextOpen = nextOpen.replace(/\s*data-zivel-service="cryo"/g, '');
  nextOpen = nextOpen.replace(/\s*zivel-service--cryo/g, '');
  ...
  if (/className=/.test(nextOpen)) {
    nextOpen = nextOpen.replace(/className="([^"]*)"/, (mm, inner) => {
      const cleaned = inner.replace(/\s*zivel-service-page\s*/g, ' ').trim();
      return `className="${cleaned} zivel-service-page"`;
    });
  } else {
